                    ...
                    <form @submit.prevent="submit" class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden mb-20">
                        <div class="p-10 space-y-8">
                            ...
                            <div v-for="setting in settings[activeTab]" :key="setting.id" class="space-y-2 p-6 bg-gray-50/50 rounded-2xl border border-gray-50 group hover:border-gray-200 transition">
                                <div v-if="setting.key === 'cre_latitude'" class="flex justify-between items-center mb-4">
                                    <h3 class="text-xs font-black text-gray-900 uppercase tracking-widest">Coordonnées Géographiques</h3>
                                    <button 
                                        type="button"
                                        @click="detectLocation"
                                        :disabled="isDetecting"
                                        class="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all disabled:opacity-50"
                                    >
                                        <GlobeAltIcon class="h-4 w-4" :class="{ 'animate-spin': isDetecting }" />
                                        {{ isDetecting ? 'Détection...' : 'Détecter ma position actuelle' }}
                                    </button>
                                </div>
